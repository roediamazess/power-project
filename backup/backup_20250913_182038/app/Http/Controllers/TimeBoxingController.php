...
class TimeBoxingController extends Controller
{
    ...
    {
        \Log::info('TimeBoxing Store Request:', [
        ...
        try {
            $payload = $request->all();
            \Log::info('TimeBoxing Store - Processing payload:', $payload);
            ...
            // Set user_id to current authenticated user (default to 1 if no auth)
            $payload['user_id'] = auth()->id() ?? 1;
            \Log::info('TimeBoxing Store - Final payload with user_id:', $payload);

            $timeBoxing = new TimeBoxing();
            \Log::info('TimeBoxing Store - Created new TimeBoxing model');
            
            $timeBoxing->fill($payload);
            \Log::info('TimeBoxing Store - Filled model with data');
            
            $timeBoxing->save();
            \Log::info('TimeBoxing Store - Saved to database successfully', ['id' => $timeBoxing->id]);
            ...
            \Log::error('TimeBoxing Store - Exception occurred:', [
                'error_message' => $e->getMessage(),
                'error_file' => $e->getFile(),
                'error_line' => $e->getLine(),
                'error_trace' => $e->getTraceAsString(),
                'payload' => $payload ?? 'Not set'
            ]);
