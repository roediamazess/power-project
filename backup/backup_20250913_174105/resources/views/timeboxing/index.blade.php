...
<script>
    ...
    async function saveTimeBoxing(activityData) {
        try {
            ...
            if (result.success) {
                ...
                showNotification('Time Boxing created successfully', 'create', {
                    important: true,
                    duration: 5000
                });
                return result.data;
    ...
    async function updateTimeBoxing(id, activityData) {
        try {
            ...
            if (result.success) {
                ...
                showNotification('Time Boxing updated successfully', 'update', {
                    important: true,
                    duration: 4000
                });
                return result.data;
    ...
    async function deleteTimeBoxing(id) {
        try {
            ...
            if (result.success) {
                ...
                showNotification('Time Boxing deleted successfully', 'delete', {
                    duration: 4000
                });
                return true;
            } else {
    ...
    function showNotification(message, type = 'info') {
        ...
        if (!document.getElementById('notification-container')) {
            ...
            if (logo && floatingCapsule) {
                ...
                // Position notification directly below the logo in floating capsule
                logoPosition = {
                    top: logoRect.bottom + 16, // 16px gap below logo
                    left: logoRect.left // Align with logo position
                };
        ...
        notification.className = `notification-capsule notification-${type}`;
        ...
        notification.innerHTML = `
            <div style="display: flex; align-items: center;">
                <div style="background: rgba(255, 255, 255, 0.2); border-radius: 50%; width: 2rem; height: 2rem; display: flex; align-items: center; justify-content: center; margin-right: 0.75rem;">
                    ${getIcon(type)}
                </div>
                <div style="flex: 1; padding: 0 0.75rem; display: flex; flex-direction: column;">
                    <p style="font-size: 0.875rem; font-weight: 500; line-height: 1.25; margin: 0;">${message}</p>
                    <div style="height: 2px; background: ${getProgressColor(type)}; margin-top: 0.25rem; border-radius: 1px; animation: shrink 4.5s linear forwards;"></div>
                </div>
            </div>
        `;
        ...
        notification.addEventListener('click', () => {
            notification.style.animation = 'fade-out 0.4s ease-in forwards';
    ...
    function getNotificationBg(type) {
        const colors = {
            ...
            create: 'linear-gradient(135deg, #8b5cf6, #7c3aed)',
            update: 'linear-gradient(135deg, #06b6d4, #0891b2)',
            delete: 'linear-gradient(135deg, #f97316, #ea580c)'
        };
    ...
    // Enhanced notification demo on page load
    document.addEventListener('DOMContentLoaded', function() {
        console.log('🚀 Time Boxing page loaded');
        console.log('🔍 Checking enhanced notification system...');
        ...
        showNotification('Time Boxing page loaded successfully', 'info', {
            duration: 3000
        });
        
        // Demo different notification types with enhanced features
        setTimeout(() => {
            showSuccess('Welcome to Time Boxing! Ready to boost your productivity?', {
                duration: 4000
            });
        }, 1000);
        
        setTimeout(() => {
            showImportant('Tip: Use Ctrl+Click to open edit modal for detailed editing', 'info');
        }, 3000);
        
        setTimeout(() => {
            showQuick('Quick tip: Press Enter to save changes', 'success');
        }, 6000);
    });
